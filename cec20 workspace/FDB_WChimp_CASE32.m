function [bestSolution, bestFitness, l]=FDB_WChimp_CASE32(fhd, dimension, maxFE, fNumber)
...
%% % Please note that to choose various Chaotic maps you should use the related Chaotic maps strategies
            ...
            if rand <0.7
                D_Attacker=abs(C1*Positions(i,j)-m(l)*Positions(index,j));
            %                           A                    B                
                X1=Attacker_pos(j)-A1*D_Attacker;

                D_Barrier=abs(C2*Positions(index,j)-m(l)*Positions(i,j));
            %                   C                            D            
